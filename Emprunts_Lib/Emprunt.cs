...
namespace Emprunts_Lib
{
    class Emprunt : IEmprunt
    {
        ...
        public IEmprunt GetEmprunt (IDataReader dr)
        {
            ...
            emprunt.Id = Convert.ToInt32(dr[""].ToString());
            emprunt.Tier = dr[""].ToString();
            emprunt.Montant = float.Parse(dr[""].ToString());
            emprunt.MontantRemb = float.Parse(dr[""].ToString());
            emprunt.DateEmprunt = DateTime.Parse(dr[""].ToString());
            emprunt.DateRembu = DateTime.Parse(dr[""].ToString());
            return emprunt;
        }
